...
memberEmail.addEventListener("input", function(){
    ...
    // 영어 숫자 -,_로 4글자 이상 @다음에 단어 -,_로 한글자 이상 .그리고 단어 그룹을 1번에서 3번 써야함.

    if(regEx.test(memberEmail.value)){ //유요한 경우
...
memberNickname.addEventListener("input", function(){
    ...
    if(memberNickname.value.trim().length == 0){
        ...
        checkObj.memberNickname = false; // ##유효성 검사 안했다고 표시
        return;
    }
