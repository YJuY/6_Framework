...
if(imageInput !=null){
    ...
    // 해당 홤녀 진입 시 프로필 이미지 상태를 저장(initCheck)
    ...
    // *input type = "file" 요소는 값이 없을 대 ''(빈칸)
    // * input type = "file" 요소는 이전에 선택한 파일이 있어도 취소하면 다시 ''(빈칸)
    // * input type = "file" 요소로 파일을 선태하면 change 이벤트가 발생한다.

// input타입 값이 변했을 때
    imageInput.addEventListener("change", (e)=>{

        // e.target : 이벤트가 발생한 요소(== imgeInput)
        ...
        if(e.target.files[0]!= undefined){
            ...
            // 초기 이미지로 다시 변경
            profileImage.setAttribute("src",originalImage);80
            deleteCheck = -1;
        }
    });
