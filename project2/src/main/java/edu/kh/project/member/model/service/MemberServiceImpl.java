...
@Service // Service 레이어, 비즈니스 로직을 가진 클래스임을 명시 + bean 동록
public class MemberServiceImpl implements MemberService{
	
	// MemberDAO bean을 의존성 주입(DI)
	@Autowired
	private MemberDAO dao /*= new MemberDAO()*/; //### new 안쓰기 위해서 DAO에 bean뭘 해줘야함
	
	
	
	// spring-seurity.xml에서 등록한 bean을 의존성 주입(DI)
	@Autowired
	...
	public Member login(Member inputMember){
		...
		System.out.println("암호화된 비밀번호 : "+ bcrypt.encode(inputMember.getMemberPw()));
	 ...
	 *  - 예외의 기본값은 RuntimeExcetion
	 *  	-> SQL 진행 시 발생하는 예외 == SQLException(RuntimeEception 형제)
	 *  								(다형성 적용 x)
	 * 		-> 다른 예외에도 rollback이 진행되고 싶다면
	 * 			rollback = 예외클래스 작성하면 된다.
	 * 
	 * 
	 */
