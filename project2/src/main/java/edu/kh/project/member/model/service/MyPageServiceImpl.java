...
public class MyPageServiceImpl implements MyPageService{
	...
	public int updateProfile(String webPath, String filePath, MultipartFile profileImage, Member loginMember) throws Exception {
		...
		// 중복 파이명 업로드를 대비하기 위해서 파일명 변경
		String rename = null;
		
		if(profileImage.getSize() == 0) { // 업로그된 파일이 없는 경우
			...
			// /resources/imges/memberProfile/변경된파일명
...
//		result = 0;
		...
		if(result > 0) { // DB 수정 성공 시 -> 실제로 서버에 파일 저장
			...
			if(rename != null) {
				// 변경된 이미명이 있다.  == 새로운 파일이 업로드 되었다.
				
				 profileImage.transferTo(new File(filePath + rename));
				 // 메모리에 임시 저장된 팡리을 지정된 경로 파일 형태로 변환
				 // == 서버 파일 업로드
			}
