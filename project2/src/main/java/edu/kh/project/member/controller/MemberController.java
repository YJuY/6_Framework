...
//				웹 애플리케이션으로 저달 받은 클라이언트의 요청을 
//  			알맞은 서비스로 연결하고
//				서비스에서 반환된 결과에 따라
//				알맞은 화면으로 응답은 방업을 제어하는 역할
...
public class MemberController {
	...
	// 자동 연결 규칙 : 타입이 같거나 상속관걔인 bean을 자동으로 DI
...
//	@RequestMapping(value ="/member/login", method= RequestMethod.POST )
	public String login(HttpServletRequest req) {
		...
		// * forward 방법 : prefix, suffix를 제외한 나머지 jsp 결오
...
//	@RequestMapping(value ="/member/login", method= RequestMethod.POST )
	...
	// == @RequestMapping(value="/member/login", method = RequestMethod.POST)
	// @PostMapping("/membe/login") // POST 방식의 /member/login 요청을 연결
	// @GetMapping("/membe/login") // Get 방식의 /member/login 요청을 연결
	...
	public String login(/* @ModelAttribute */ Member inputMember,
						...
						HttpServletResponse resp,// 쿠기 전달용
						@RequestHeader(value="referer") String referer // 이전 요청 주소
						) { 
		
		System.out.println(saveId);
		// Model : 데이터 전달용 객체
		// - 데이터를 Map(###K:V) 형식으로 저장하여 전달하는 객체
		// - request scope가 기본값
		...
		if(loginMember != null) {
			...
			// *****************************************************
			// 쿠기 생성
			Cookie cookie = new Cookie("saveId", loginMember.getMemberEmail()); // 로그인된 멤버 이메일을 쿠키에 저장하겠다.
			...
			// new : @RequsetHaeder(value="referer")String referer
			//		 path = referer;
...
//	get방식 형식으로 멤버 로그인 요청
	...
	public String logout(SessionStatus status) {
		...
		// 왜? @SessoionAttributes로 session scope에 등록된 값을 무효화 시키려면
...
//	#######Member inputMember : 여기에 입력했던 모든 값들이 다 들어 ㅣㅇㅆ을 것이다
...
//			#####멤버어드레스가 스트링 배열이면 하나의 문자열로 합쳐라
		...
		if(result > 0) { // 성공 시
			...
			inputMember.setMemberPw(null); // 비밀번호 삭제(보안문제)
	...
	/* 스프링 예외 처리 방법 (3종류, 붕복 사용 가능)
	 * 
	 * 1순위 : try-catch / throws 예외처리 구문
	 * 		 -> 메서드 단위로 처리
	 * 
	 * 2순위 : @ㄸExceptionHandler 어노테이션
	 * 		 -> 클래스 단위로 처리
	 * 		 - 하나의 컨트롤러에서 발생하는 예외를
	 * 		   하나의 메서드로 모아서 처리
	 * 
